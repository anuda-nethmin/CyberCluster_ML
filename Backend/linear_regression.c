                            ...
                            double *loss_history)
{
    ...
    // 3. If the file exists and dimensions match, load the old weights!
    if (fp_in && fread(&saved_dim, sizeof(int), 1, fp_in) == 1 && saved_dim == dim) {
    fread(weights, sizeof(double), dim + 1, fp_in);
    fclose(fp_in);
    } else {
